...
module.exports.saveEditpost = async (req, res) => {
    ...
    try {
        ...
        // Get the existing listing first to have fallback coordinates
        ...
        try {
            ...
            if (geoData && geoData.length > 0) {
                ...
                // No geocoding results, check if existing geometry is valid
                console.warn(`No geocoding results for location: ${location}. Checking existing coordinates.`);
                
                if (editList.geometry && editList.geometry.type && editList.geometry.coordinates && editList.geometry.coordinates.length === 2) {
                    updatedGeometry = editList.geometry;
                } else {
                    console.warn("Existing geometry is invalid. Using default coordinates.");
                    updatedGeometry = {
                        type: 'Point',
                        coordinates: [0, 0]
                    };
            ...
            console.warn('Using existing or default coordinates due to geocoding failure');
        ...
        // Handle new image uploads - append to existing images instead of replacing
        if (req.files && req.files.length > 0) {
            req.files.forEach(file => {
              // Upload to Cloudinary and append to existing images
                editList.image.push({
                    url: file.path,
      ...
      // Update other fields
        ...
        // Log the geometry before saving
        console.log('Updated geometry:', JSON.stringify(updatedGeometry));
        
        editList.geometry = updatedGeometry; // Save the GeoJSON object in geometry
        editList.markModified('geometry'); // Mark as modified for Mongoose
        
        // tags
        editList.tags = tagArray; // Save the updated tags
        
        console.log('About to save listing with geometry:', JSON.stringify(editList.geometry));
...
module.exports.topListings = async (req, res) => {
    try {
        ...
        const topRatedListings = listings.filter(listing => {
            if (!listing.reviews || listing.reviews.length === 0) return false; // Skip listings with no reviews
            
            // Filter out any null/undefined reviews that might have been deleted but still referenced
            const validReviews = listing.reviews.filter(review => review && review.rating !== undefined);
